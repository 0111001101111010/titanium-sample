...
NSString * PlatformerDemo$ModuleRequireFormat = @"(function(exports){"
...
@implementation PlatformerDemoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_platformerdemo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create PlatformerDemo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "PlatformerDemo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_platformerdemo = [[PlatformerDemoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_platformerdemo];
	
	NSString *_platformerdemoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _platformerdemoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_platformerdemo valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:PlatformerDemo$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.platformerdemo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.platformerdemo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
