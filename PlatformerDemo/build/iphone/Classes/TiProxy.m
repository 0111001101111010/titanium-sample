...
//Rare exceptions to indicate a bug in the _platformerdemo code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_platformerdemo = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_platformerdemo]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.platformerdemo.module"
